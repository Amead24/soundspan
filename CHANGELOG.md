...
- Vibe-map queue ergonomics, from live play-testing: the queue panel gained a **Clear queue** button (same Listen-Together-aware `clearQueue` primitive and toast as /queue's — it stops playback too) and its upcoming rows are now **click-to-jump** (`playQueueIndex` verbatim; the drag handle still reorders, ✕ still removes); the now-playing card gained a **skip-next** button that renders only while a next queue item exists — in fullscreen the card is the only transport, so skipping no longer requires leaving the map.
