...
- New `GET /api/vibe/mixer/:trackId` returns every cached-map track's CLAP and lyric-semantic similarity against a seed track as arrays index-aligned to the map projection's `tracks` order (4dp, `null` where an embedding is missing or lyrics don't apply to the pair) — the data the map's live weight-mixer recoloring blends client-side. Exact scans bounded by the 15k projection cap, not ANN. Responds 409 `{stale:true}` when no projection is cached (client loads the map first) and carries the projection's `computedAt`/`count` so clients can detect drift; responses are Redis-cached per seed+projection (`vibe:mixer:v1:*`, 1h TTL, self-invalidating since the key embeds `computedAt`).
