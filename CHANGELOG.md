...
- Vibe-map dots grew a **Ctrl+Shift-click = play next** gesture (also on travel halos and travel-panel neighbour rows): the track is inserted right after the current song via the player's Listen-Together-aware `playNext` primitive, while plain Shift-click keeps appending to the queue's end — born from play-testing ("I see a dot I want next, but shift-click buries it at the bottom of the queue"). The explore/travel hint chips and row tooltips teach the new verb; ctrl-click blending and the journey destination pick keep their precedence, with the intercept order pinned by behaviour tests.
