...
- The vibe map's modifier-click grammar was redesigned around two play-test findings ("I see a dot I want next, but shift-click buries it at the bottom of the queue" and "ctrl-click surprise-teleported me into the alchemy tray"). **Alchemy is now opt-in**: a flask button in the map controls opens the workspace sidebar (empty tray, stays open when the last ingredient is removed, disabled during journey — the same half-built-route protection the old toast gave); while it's open, plain and ctrl-clicks add ingredients. With it closed — the default — **ctrl-click (or ctrl+shift-click) queues the dot as the next song** via the player's Listen-Together-aware `playNext` primitive, on canvas dots, travel halos and travel-panel neighbour rows alike; plain Shift-click keeps appending to the queue's end, and no modifier click can silently switch modes anymore. Hint chips, tooltips and the tray's helper copy teach the new verbs; the journey destination pick keeps its intercept, and the whole precedence order is pinned by behaviour tests.
- The vibe surfaces are now reachable from the main navigation: **Vibe Explore** (`/vibe`) and **Vibe Map** (`/vibe?tab=map`) entries in the desktop sidebar and the mobile drawer's quick links. The vibe page's Explore/Map tab became deep-linkable via `?tab=map` — in-page tab switches mirror into the URL with `router.replace` (no history stacking, unrelated params preserved, and the sidebar highlight follows because only the router notifies other `useSearchParams` subscribers — native `history.replaceState` verifiably does not). Active-state matching is query-aware via a new `getActiveNavHref` helper (an item matches when its path and every query param it declares match; the most specific match wins). Also removed the sidebar's dead mobile-header block — `sidebarContent` only ever renders in the desktop `<aside>`; mobile gets the separate `MobileSidebar` drawer, which already carries its own header and sync button.
- Vibe-map queue ergonomics, from live play-testing: the queue panel gained a **Clear upcoming songs** button backed by a new `clearUpcoming()` audio-controls primitive — it drops only the items after the current one and the music keeps playing (the first cut reused /queue's `clearQueue`, whose clear-and-stop semantics surprised in a "what's next" panel; /queue's explicit Clear Queue button keeps them). Disabled when nothing is upcoming, hidden during Listen Together (the server-owned queue has no upcoming-only operation — same policy as hiding reorder). The panel's upcoming rows are also now **click-to-jump** (`playQueueIndex` verbatim; the drag handle still reorders, ✕ still removes), and the now-playing card gained a **skip-next** button that renders only while a next queue item exists — in fullscreen the card is the only transport, so skipping no longer requires leaving the map.
- The vibe map gained a **song x-ray** (ArrowLeftRight button in the view-controls stack): pick two songs — in-panel search plus "Now playing" / "Mixer seed" shortcuts — and get the component-by-component answer to "why do these (not) match": overall match under your weight mix (labeled default/custom), true sonic (CLAP) percent, dual-marker gap bars for each audio feature, raw BPM values side by side, Camelot key badges (8A/9B-style), a lyric block (theme match %, word-mood on the −1…+1 scale, MTLD vocabulary scores — with an explicit "lyric dials don't apply" state when a side is instrumental or unanalyzed), clickable shared-neighbor chips, and **plain-English verdict sentences** ("Clearly alike in sound… but energy differs by 50% — that gap is what your ears notice", "Opposite emotional polarity in the words" — the happy-sounding-sad-song detector). The "?" About popover also grew a Dimensions section explaining all 12 similarity dimensions in listener terms (including the honest English-centric disclaimer on the lyric metrics), so twisting knobs comes with understanding what they mean.
- The vibe map gained a **similarity mixer** (SlidersHorizontal button in the view-controls stack — a new aux surface alongside queue/trail/about): pick a seed track via in-panel search or "Use now playing", then drag 12 dimension sliders — Sound (CLAP), Energy, Brightness, Tempo, Groove, Acoustic, Instrumental, Key, and four lyric dials (meaning / mood / vocabulary / complexity) — and the whole map recolors **live while dragging** as a heatmap of blended similarity to the seed (slate → amber color ramp with score-scaled opacity; dots with no data for the pair render dim gray). You are literally turning the recommender's knobs: the same weights persist per-user (debounced save, reset button) and drive server-side `/similar` scoring, so Travel-mode suggestions obey the mix you set here. A "Closest under this mix" top-20 list re-ranks as you drag (click a row to fly to its dot), a lyric-coverage line shows how many songs the lyric dials can actually see, and an **attract/repel force** toggle (with strength) morphs the layout itself — similar dots drift toward your seed or dissimilar ones push away, through the same eased position animation the Spread toggle uses, snapping back when turned off. Blend math renormalizes over available components, so lyric-less tracks compete fairly on audio terms instead of sinking when lyric dials are up.
...
- The six vibe aux panels (Travel, Journey, Alchemy, Queue, Mixer, X-ray) now render through one shared `<VibePanel>` primitive (`components/vibe/panelChrome.tsx`, which also becomes the neutral home of the chrome constants that used to live opt-in inside TravelPanel — the exact arrangement that let two panels ship as mis-anchored mobile sheets). The primitive owns the container, header, and ✕ button, so the paired class+style contract can no longer be half-adopted, and it adds the panels' first real focus contract: opening a panel moves focus to its heading, and closing it (✕, Esc, or programmatic) returns focus to the control that opened it instead of dropping to `<body>`. Every panel title is now a real `<h3>` heading. The mixer seed picker and both x-ray A/B pickers also collapse into one shared `TrackSearchPicker` (input + ranked dropdown + shortcut chips), removing ~70 lines of duplicated dropdown with per-panel deltas.
- The vibe map payload is now **v5**, dieting away essentially all of v4's growth (review findings A3/A10/C4): `moodScore` (read by no client code) and the 7-float per-track `moods` record are replaced by the single `moodHappy` scalar the travel compass actually consumes, and `x`/`y` ship at 4dp (sub-pixel on any real screen) — measured on the real 15,237-track corpus, the equivalent diet brings a fully-lyric-backfilled payload from 9.9MB raw / 2.29MB gzipped back to ~8.0MB raw / ~1.9MB gzipped. Serving also got structurally cheaper: `/api/vibe/map` cache hits now stream the cached JSON bytes as-is instead of `JSON.parse`ing and re-stringifying the multi-MB payload per request, and `/api/vibe/mixer/:trackId` reads a new slim **ordered** ids companion key (`vibe:map:v5:ids` — `{computedAt, trackCount, ids}`, written beside the projection, unlike the deleted write-only v3-era unordered SET) instead of parsing the full projection on every request including mixer-cache hits — previously the endpoint's dominant cost (~50-100ms + allocation churn). Cache keys moved `vibe:map:v4:*` → `vibe:map:v5:*`; old keys expire via their 24h TTL, no manual step. The frontend `MapTrack`/travel-compass types follow the flatten (`moods?.moodHappy` → `moodHappy`).
- New `GET /api/vibe/xray?a=&b=` compares any two tracks component by component ("more explainability between songs"): true CLAP cosine similarity, each audio-feature similarity (`bpm_similarity`/`key_similarity` reuse the scoring SQL functions so the x-ray never disagrees with the recommender), raw key/scale for client-side Camelot labeling, lyric semantic similarity plus sentiment/lexical/reading pairs (nulled with explicit `analyzed`/`instrumental`/`unknown` statuses when lyrics don't apply), up to 5 shared neighbors (the intersection of both tracks' similar-lists under the requester's weight mix), and an overall §blend score under the requester's weights labeled `default`/`custom`. Machine-readable by design — the UI composes the plain-English verdicts.
- New `GET /api/vibe/mixer/:trackId` returns every cached-map track's CLAP and lyric-semantic similarity against a seed track as arrays index-aligned to the map projection's `tracks` order (4dp, `null` where an embedding is missing or lyrics don't apply to the pair) — the data the map's live weight-mixer recoloring blends client-side. Exact whole-table scans over the ~15k embedding rows, not ANN (and deliberately not id-filtered: a `= ANY(<15k ids>)` bind ships ~390KB of parameters per scan, wire-times ~1.5× slower than the plain scan on the production pg-driver path — 36ms vs 25ms median — and degrades to ~73ms under a generic plan, all while producing the same response). Responds 409 `{stale:true}` when no projection is cached (client loads the map first) and carries the projection's `computedAt`/`count` so clients can detect drift; responses are Redis-cached per seed+projection (`vibe:mixer:v1:*`, 1h TTL, self-invalidating since the key embeds `computedAt`).
- Similar-track scoring is now user-tunable ("weight mixer" backend): new `GET`/`PUT /api/vibe/weights` store a per-user 12-component similarity mix (`clap`, the 7 audio features, and 4 new lyric components — `lyricSemantic`/`lyricSentiment`/`lyricLexical`/`lyricReading`; zod-validated, each 0..1 with at least one positive; `PUT null` resets). `GET /api/vibe/similar/:trackId` — and everything built on it, like the vibe map's Travel mode — resolves the caller's stored mix and threads it into `findSimilarTracks`. With no stored mix the defaults are numerically identical to the previously hardcoded weights (`0.55·clap + …`), emitting the same SQL parameters — untouched users see byte-for-byte the old ordering, which the existing behavior suites pin. When any lyric weight is positive, the hybrid query LEFT JOINs the lyric tables and blends the four lyric similarities under a single availability gate (both tracks lyric-analyzed and non-instrumental) with per-row renormalization, so lyric-less tracks compete fairly on audio terms instead of being penalized by unfillable weight; the candidate pool remains CLAP-ANN-ordered (documented v1 limitation: lyric-heavy mixes re-rank a sonic pool). Features-only fallback mode renormalizes a custom mix over the 7 audio features.
- The vibe map payload (`GET /api/vibe/map`) is now v4: each track additionally carries `bpm`, `danceability`, `acousticness`, `key`/`keyScale`, the lyric-analysis scalars (`sentiment`, `lexicalDiversity`, `readingLevel` — null until the lyrics pipeline has analyzed the track) and a `hasLyrics` flag (true only for completed, non-instrumental lyric analysis), all floats rounded to 3dp to bound payload growth (measured against a real 15,237-track library: +2.8MB raw / +0.30MB gzipped over v3 today, growing to +0.50MB gzipped once lyric scalars are fully backfilled — v3 baseline 7.0MB raw / 1.79MB gzipped). The Redis projection cache moved from `vibe:map:v3:*` to `vibe:map:v4:*` keys — old v3 entries simply expire via their 24h TTL; no manual invalidation step. A new `getCachedProjection()` helper exposes the cached (ordered) projection for index-aligned consumers. UI behavior is unchanged in this step; the fields feed the upcoming weight-mixer/x-ray features.
...
- Every vibe endpoint that binds a query embedding into SQL — the mixer's scans, journey/alchemy/path routing (`findNearestToEmbedding`), and text vibe search — 500ed with Postgres `22P02` ("invalid input syntax for type vector") under the Prisma 7 pg driver adapter: a raw JS `number[]` parameter now binds as a Postgres ARRAY text literal (`{"0.1",...}`), which `::vector` rejects, where the old Rust engine bound a typed `float8[]` that pgvector could cast. All nine interpolation sites now serialize through a new `toVectorLiteral()` helper (pgvector's `[0.1,...]` text form), with transport pins in the mixer/journey/search suites asserting the bound parameter is the text literal, never an array. Found live (every mixer seed pick failed); note the same regression exists on upstream `main` for its own `/vibe/path` and `/vibe/search` since the Prisma 7 migration (#162) — `/similar` escapes it only because it dereferences the seed embedding server-side.
- The weight mixer's reset ("Reset all sliders to the defaults") 500ed on every click: the documented "`PUT /api/vibe/weights` with `null` resets" contract never actually worked over HTTP — Express 5's strict JSON parser rejects a literal `null` body before the handler runs (the handler-level tests drove the route directly and stayed green, hiding the outage). The client now sends the reset as a body-less PUT, and the handler's reset branch accepts all three empty-transport shapes (`undefined`, `null`, and the `{}` Express 5 yields for a zero-length JSON body — unambiguous, since a valid mix always carries all 12 keys). A new supertest tier runs the route through the real JSON body parser so this class can't silently regress.
- The vibe map's now-playing card and dot-hover popover now actually show album art: both rendered the payload's `coverUrl` **raw** as `<img src>`, but that value is a cover *identifier* (`native:<id>.jpg`), not a URL — every player surface resolves it through `api.getCoverArtUrl(...)` (which builds the servable, token-carrying URL) and now these two do as well. The card stays presentational: the connected wrapper resolves the URL and passes it as a new `coverUrl` prop, mirroring the mini player's `useMediaInfo` idiom.
- The x-ray's dual-marker gap bars no longer read as broken when two tracks agree perfectly about a low value (e.g. two rock tracks both ~0 acousticness showed one lonely dot at the far left next to "100%"): coinciding A/B markers now fuse into a single half-indigo/half-amber dot ("both tracks are here"), and distant markers get a connecting gap span — the highlighted span literally is the gap the row's percentage scores.
...
- The enrichment worker gained a `lyrics` phase (after `vibe`, before `podcasts`), gated by the new `LYRICS_ANALYSIS_ENABLED` flag: it bulk-fetches lyrics for tracks with no `TrackLyrics` row yet (500 per cycle through the existing LRCLIB waterfall and its rate limiter — a 15k library backfills over cycles in the background), classifies no-lyrics rows as `instrumental` so they're excluded from lyric analysis, sweeps stale `processing` rows back to NULL after 30 minutes (3 strikes → `failed`, mirroring the vibe cleanup's conventions), and queues analyzable rows onto `lyrics:analysis:queue` — but only when a live `lyrics:worker:heartbeat` proves a lyric worker exists to drain them (there is deliberately no bundled-script fallback: an older CLAP sidecar image has no lyric worker). Jobs are `{trackId}`-only and RPUSHed *before* the row flips to `processing`, so a dropped enqueue can't strand a row. Two new internal callbacks (`POST /api/analysis/lyrics/failure`, `POST /api/analysis/lyrics/success`, `x-internal-secret`-guarded) record and resolve `EnrichmentFailure(entityType='lyrics')` rows, mounted alongside the vibe callbacks so they stay reachable when `AUDIO_ANALYSIS_ENABLED=false`. No consumer of the analysis results ships yet — the phase only fetches, classifies, and queues.
- The CLAP sidecar now carries a lyric analysis worker (`services/audio-analyzer-clap/lyric_analysis.py`, `LYRIC_WORKERS` threads, default 1): it BLPOPs `lyrics:analysis:queue`, re-reads lyric text from `TrackLyrics`, and writes — in one transaction — three scalars (VADER `sentiment`, MTLD `lexicalDiversity` with the length-robust McCarthy–Jarvis bidirectional formula, Flesch-Kincaid `readingLevel` computed line-as-sentence since unpunctuated lyric sheets otherwise score as one ~grade-130 "sentence") plus a 768-D `nomic-embed-text-v1.5` embedding into `track_lyric_embeddings`. The model and its `trust_remote_code` module repo (`nomic-ai/nomic-bert-2048` — the config `auto_map` points cross-repo) are baked into the image's HF cache at pinned immutable revisions and loaded `local_files_only`, with a build-time smoke test that embeds a probe string and asserts a 768-D unit vector so a transformers-compat break fails the build, not the container. The model lazy-loads on first job and idle-unloads (shared `MODEL_IDLE_TIMEOUT`), publishes `lyrics:worker:heartbeat`, and reports failures/successes to the backend's `/api/analysis/lyrics/*` callbacks with `X-Internal-Secret`. Image grows ~550MB; the AIO image gets the same worker and model bake. Both hash-pinned dependency locks gained `einops`, `vaderSentiment`, `textstat` (+their transitive deps) without upgrading any existing pin.
- Database migration `20260716101000_add_user_similarity_weights` adds nullable `User.similarityWeights JSONB` (metadata-only `ADD COLUMN`, instant; idempotency-guarded) storing each user's weight-mixer settings; NULL means defaults.
- New admin endpoint `POST /api/analysis/lyrics/retry` un-strands failed lyric analyses: it resets every `TrackLyrics` row with `analysisStatus='failed'` back to `pending` — clearing the stored error and, deliberately, the retry count (a status-only reset would leave max-retried rows permanently skipped by the 3-strike stale sweep, the trap the Essentia `retry-failed` endpoint documents) — and bulk-resolves the matching `EnrichmentFailure(entityType='lyrics')` records. It does **not** enqueue directly: the enrichment lyrics phase re-queues pending rows on its next cycle behind the lyric-worker heartbeat gate, keeping one owner for the RPUSH-then-mark-processing discipline. Driven off the `TrackLyrics` table rather than failure records, so rows whose failure callback never landed (e.g. `INTERNAL_API_SECRET` mismatch) are still recoverable. Mirrors `POST /api/analysis/vibe/retry`, closing the gap where one transient error (cold-model hiccup, OOM blip, DB flake) excluded a track from lyric analysis forever. Lives in the always-mounted internal-callbacks router, so it stays reachable when `AUDIO_ANALYSIS_ENABLED=false` — the lyrics pipeline runs off its own `LYRICS_ANALYSIS_ENABLED` flag, same reasoning as the lyrics failure/success callbacks.
- `docker-compose.yml` raises the `audio-analyzer-clap` memory limit from 3G to 4G: the container now keeps two models resident under load — the CLAP checkpoint (2.35GB, 2-3GB peak during audio embedding) plus the lyric worker's nomic-embed model (~0.5GB+ with torch overhead) — and both queues drain concurrently after a deploy, so the 3G cap sized for CLAP alone risked OOM-kills during the initial lyric backfill. The Helm chart default was already 5Gi and is unchanged; operators with hand-tuned compose overrides should mirror the bump before running the backfill.
- Two additive, idempotency-guarded database migrations lay the (currently dark) groundwork for lyric analysis: `20260716100000_add_lyric_analysis` adds nullable result columns (`sentiment`, `lexicalDiversity`, `readingLevel`, `isInstrumental`) and `vibeAnalysisStatus`-style lifecycle columns (`analysisStatus` et al.) to `TrackLyrics` — all nullable or constant-default, so the `ADD COLUMN`s take the metadata-only fast path; `20260716100500_add_track_lyric_embeddings` creates `track_lyric_embeddings` (`vector(768)`, PK `track_id`, cascade delete) for semantic lyric embeddings (`nomic-embed-text-v1.5`), deliberately without an ivfflat index since every planned consumer is a pairwise comparison, a candidate re-rank, or a bounded exact scan (sidestepping the F14 probes-recall trap). A new `LYRICS_ANALYSIS_ENABLED` feature flag (default on, surfaced as `config.features.lyricsAnalysis`) will gate the pipeline; nothing reads any of this yet — zero behavior change.
