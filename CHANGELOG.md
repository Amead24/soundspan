...
- The vibe map payload (`GET /api/vibe/map`) is now v4: each track additionally carries `bpm`, `danceability`, `acousticness`, `key`/`keyScale`, the lyric-analysis scalars (`sentiment`, `lexicalDiversity`, `readingLevel` — null until the lyrics pipeline has analyzed the track) and a `hasLyrics` flag (true only for completed, non-instrumental lyric analysis), all floats rounded to 3dp to bound payload growth (~+0.5-0.8MB raw for 15k tracks, marginal gzipped). The Redis projection cache moved from `vibe:map:v3:*` to `vibe:map:v4:*` keys — old v3 entries simply expire via their 24h TTL; no manual invalidation step. A new `getCachedProjection()` helper exposes the cached (ordered) projection for index-aligned consumers. UI behavior is unchanged in this step; the fields feed the upcoming weight-mixer/x-ray features.
